---
name: tcases.create
description: Generate individual test case files from an existing test plan. Use after /tplan.create to produce TC-*.md files, INDEX.md, and update the test plan.
user-invocable: true
model: opus
...
```
/tcases.create [FEATURE_DIR]
```

Examples:
- `/tcases.create` (auto-detects from prior `/tplan.create` run)
- `/tcases.create mcp_catalog`
- `/tcases.create /path/to/feature_dir`
...
### Auto-detection
If no arguments are provided and a TestPlan.md was just generated by `/tplan.create` in this session, use its feature directory automatically — proceed directly to Step 1.
...
1. Check if `<feature_dir>/TestPlanGaps.md` exists (generated by `/tplan.create`)
...
- Does NOT modify the test plan's Sections 1-4, 6-7 — those are owned by `/tplan.create`
