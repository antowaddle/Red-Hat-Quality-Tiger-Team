...
description: Generate individual test case files from an existing test plan. Use after /test-plan-creator to produce TC-*.md files, INDEX.md, and update the test plan.
user-invocable: true
model: opus
...
```
/test-cases-creator [FEATURE_DIR]
```

Examples:
- `/test-cases-creator` (auto-detects from prior `/test-plan-creator` run)
- `/test-cases-creator mcp_catalog`
- `/test-cases-creator /path/to/feature_dir`
...
### Auto-detection
If no arguments are provided and a TestPlan.md was just generated by `/test-plan-creator` in this session, use its feature directory automatically — proceed directly to Step 1.
...
1. Check if `<feature_dir>/TestPlanGaps.md` exists (generated by `/test-plan-creator`)
...
- Does NOT modify the test plan's Sections 1-4, 6-7 — those are owned by `/test-plan-creator`
